...
export default function ClassDetail() {
  ...
  return (
    <div className="space-y-10">
      <header className="h-40 border-b border-brand-border flex items-end px-4 md:px-10 pb-8 bg-white -mx-4 md:-mx-10 -mt-4 md:-mt-10 overflow-hidden relative">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-2">
            <Link 
              to="/" 
              className="p-2 border border-brand-border hover:bg-brand-bg rounded-full transition-colors"
            >
              <ArrowLeft className="w-4 h-4 text-brand-text" />
            </Link>
            <div className="flex items-center gap-3">
              <span className="text-[10px] uppercase tracking-[0.2em] font-bold text-blue-600">{cls.section || 'Engineering'} / SY 2024</span>
              <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[9px] font-bold rounded uppercase">Join Code: {cls.join_code}</span>
            </div>
          </div>
          <h1 className="text-5xl font-serif italic tracking-tight text-brand-text">{cls.name}</h1>
        </div>
        <div className="flex items-center gap-6">
          ...
          <button className="w-12 h-12 bg-brand-text text-white rounded-full flex items-center justify-center text-xl shadow-lg transform rotate-45">
            <Plus className="w-6 h-6 transform -rotate-45" />
          </button>
        </div>
      </header>

      <nav className="flex gap-10 border-b border-brand-border pb-px">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as Tab)}
            className={`
              text-[11px] font-bold uppercase tracking-[0.2em] pb-3 transition-all relative
